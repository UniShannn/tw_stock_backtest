...
def send_line_message(message_text, group_name ="工作紀錄"):
    ...
    # 設定要傳送的資料內容
    data = {
        'to': config.LINE_GROUP_IDs[group_name],  # 這裡指定要發給自己的 User ID
        'messages': [
            {
                'type': 'text',
                'text': message_text
...
if __name__ == "__main__":
    ...
    # 呼叫函數，將字串傳出去
    send_line_message(backtest_result, group_name="工作紀錄")