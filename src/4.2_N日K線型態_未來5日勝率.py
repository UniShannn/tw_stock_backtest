...
def calculate_n_days_pattern_win_rate(n_days=3, min_occurrence=30):
    ...
    for file_path in all_files:
        try:
            ...
            # 1. 將單日漲跌幅轉換為等級 (若您的資料已包含'漲跌幅等級'欄位，這步可省略)
            df['漲跌幅等級'] = df['漲跌幅'].apply(categorize_return)
            
            # 2. 單日狀態拼接 (K線 + 漲跌幅) -> 例如: "[11.綠K|09.-4~-2%]"
            df['Daily_Status'] = "[" + df['K線型態(簡化)'] + "|" + df['漲跌幅等級'] + "]"
            ...
            df['N日累計漲跌幅等級'] = df['N日累計漲幅數值'].apply(categorize_return)
